...
def step(t:float):
    for i in ti.grouped(x):
        v[i].y -= gravity*dt
    for i in ti.grouped(x):
        force = ti.Vector([0.3, 0.0, 0.0])*ti.abs(ti.sin(t))
    ...
    for i in ti.grouped(x):
        ...
        if (x[i]-ball_center[0]).norm() <= ball_radius+intersection_offset:
            v[i]=[0,0,0]
        x[i] += dt*v[i]
...
if __name__ == "__main__":
    ...
    while window.running:
        ...
        scene.particles(ball_center, radius=ball_radius, color=(1.0, 1.0, 1.0))
        canvas.scene(scene)
        window.show()
